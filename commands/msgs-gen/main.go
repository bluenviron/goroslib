...
func processPackage(name string, addr string) error {
	...
	err = shellCommand(fmt.Sprintf("go run ./commands/msg-import --gopackage=%s --rospackage=%s --comment=\"%s\" %s > %s",
		name,
		name,
		"Package "+name+" contains message definitions (autogenerated).",
		strings.Join(urls, " "),
		filepath.Join("msgs", name, "package.go")))
	if err != nil {
		return err
	}
